...
def get_number(line:str, ind:int) -> int:
    return_num:int = 0
    for num in re.finditer(r"\d+", line):
        if ind in range(num.start(), num.end()):
            return_num =  int(num.group())

    if return_num == 0:
        print(f"couldn't find number at position {ind} for line {line}")
    return return_num

def check_around(row_num:int, start_index:int, row_position:str) -> list[int]:
    return_nums:list[int] = []
    row_to_check:str
    if row_position == "up":
    ...
    middle_found = False
    if row_to_check[start_index].isdigit():
        middle_found = True
        return_nums.append(get_number(row_to_check, start_index))
    
    if not middle_found:
        if start_index != 0:
            if row_to_check[start_index-1].isdigit():
                return_nums.append(get_number(row_to_check, start_index-1))
        
        if start_index != len(row_to_check)-1:
            if row_to_check[start_index+1].isdigit():
                return_nums.append(get_number(row_to_check, start_index+1))
    
    return return_nums

def check_gear(row_num:int, start_index:int) -> int:
    global DATA
    ratios:list[int] = []
    gear_ratio:int = 0
    
    if start_index != 0:
        if DATA[row_num][start_index-1].isdigit():
            ratios.append(get_number(DATA[row_num], start_index-1))

    if start_index != len(DATA[row_num])-1:
        if DATA[row_num][start_index+1].isdigit():
            ratios.append(get_number(DATA[row_num], start_index+1))
    
    if row_num != len(DATA)-1:
        ratios.extend(check_around(row_num, start_index, "up"))
    
    if row_num != 0:
        ratios.extend(check_around(row_num, start_index, "down"))
...
def main():
    ...
    for row_num, row_str in enumerate(DATA):
        gears = re.finditer(r"\*", row_str)
        for gear in gears:
            gear_ratios.append(check_gear(row_num, gear.start()))
