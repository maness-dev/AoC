...
COORDS = ["14gxqgqsqqbxfpxnbccjc33eight",
"eight2sevenkl",
...
def get_first_dig(line:str) -> int:
    dig:int = 0
    for char in line[::]:
        if char.isdigit():
            dig = int(char)
...
def parse_front_str_num(line:str) -> str:
    global SPELLED_NUMS
    new_line:str = ""
    found_string:bool = False
    for ind, char in enumerate(line[::]):
        if char.isdigit():
            break
        found:bool = False
        for num_str in SPELLED_NUMS:
            if num_str[0] == char:
                if str(line[ind:ind+len(num_str)]) == num_str:
                    new_line = f"{line[:ind]}{SPELLED_NUMS[num_str]}{line[ind+len(num_str):]}"
                    found = True
                    found_string = True
                    break
        if found:
...
def parse_back_str_num(line:str) -> str:
    global SPELLED_NUMS
    new_line:str = ""
    found_string:bool = False
    for ind, char in enumerate(line[::-1]):
        ...
        for num_str in SPELLED_NUMS:
            last = len(num_str)-1
            if num_str[len(num_str)-1] == char:
                sub = line[:len(line)-ind-len(num_str)]
                if str(line[len(line)-ind-len(num_str):len(line)-ind]) == num_str:
                    new_line = f"{line[:len(line)-ind-len(num_str)]}{SPELLED_NUMS[num_str]}{line[len(line)-ind:]}"
                    found = True
                    found_string = True
                    break
        if found:
...
def convert_string_nums(line:str) -> str:
    new_line:str = ""
    parsed_front = parse_front_str_num(line)
    new_line = parse_back_str_num(parsed_front)
    return new_line
...
def main():
    ...
    nums:list[int] = []
    for coord in COORDS:
        updated_coord = convert_string_nums(coord)
        first = get_first_dig(updated_coord)
        last = get_last_dig(updated_coord)
