...
def parse_string(line:str):
    ...
    game_line:str = parts[0]
    nums = re.findall(r"\d+",game_line)
    game_num:int = int(nums[0]) if len(nums) > 0 else 0
    ...
    for set in sets:
        colors = set.split(",")
        for color in colors:
            counts = re.findall(r"\d+",color)
            color_count:int = int(counts[0]) if len(counts) > 0 else 0
            if "green" in color:
                if color_count > criteria["green"]:
                    return
            if "red" in color:
                if color_count > criteria["red"]:
                    return
            if "blue" in color:
                if color_count > criteria["blue"]:
                    return
