...
def bit_flip(xy):
    global lights
    lights[xy] = lights[xy]+2

def state_change(xy,state):
    global lights
    if state == 'on':
        lights[xy] = lights[xy]+1
    else:
        if lights[xy] != 0:
            lights[xy] = lights[xy]-1

for line in instructions:
    operation, x1, y1, x2, y2 = parse_line(line)

    x_diff = x2-x1
    y_diff = y2-y1

    for x_update in range(x1,x1+x_diff+1):
        for y_update in range(y1, y1+y_diff+1):
            xy = f"{x_update},{y_update}"
            match operation:
                case 'on': state_change(xy,'on')
                case 'off': state_change(xy,'off')
                case 'tog': bit_flip(xy)
brightness = 0
for coord in lights:
    brightness += lights[coord]
